...
class ElevatorDetailByMachineNumberViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.elevator = Elevator.objects.create(
            ...
            machine_number="1892389523492545",
            capacity=10,
            manufacturer="Hawaii",
    ...
    def test_get_elevator_details_invalid_machine_number(self):
        ...
        url = reverse('maintenance_companies:elevator-detail-by-machine-number', 
                     kwargs={
                         'company_id': str(self.company.id),
                         'machine_number': '9999999999999999'
                     })
    ...
    def test_get_elevator_details_empty_machine_number(self):
        ...
        url = f'{self.base_url}/{self.company.id}/ /'
        response = self.client.get(url)
    ...
    def test_get_elevator_details_invalid_uuid_format(self):
        ...
        # Don't use reverse() for invalid UUID - construct URL directly
        url = f'{self.base_url}/invalid-uuid/123456/'
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_get_elevator_details_non_numeric_machine_number(self):
        """Test GET request with non-numeric machine number."""
        url = reverse('maintenance_companies:elevator-detail-by-machine-number',
                     kwargs={
                         'company_id': str(self.company.id),
                         'machine_number': 'abc123'
                     })
        
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn('Machine number must be numeric', str(response.data))

    def test_get_elevator_details_whitespace_machine_number(self):
        """Test GET request with whitespace-only machine number."""
        url = f'{self.base_url}/{self.company.id}/   /'
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn('Machine number is required', str(response.data))
