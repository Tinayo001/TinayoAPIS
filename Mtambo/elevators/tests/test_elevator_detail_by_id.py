...
class ElevatorDetailByIdViewTest(APITestCase):
    def setUp(self):
        """
        Set up test data.
        """
    ...
    def test_retrieve_elevator_by_id(self):
        """
        Test retrieving an elevator by its ID (happy path).
        """
        url = reverse('elevator-detail', args=[self.elevator.id])
    ...
    def test_retrieve_nonexistent_elevator(self):
        ...
        url = reverse('elevator-detail', args=[nonexistent_id])
        response = self.client.get(url)
